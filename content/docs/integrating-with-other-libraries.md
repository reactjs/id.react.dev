...
Lihat bagaimana kita membungkus `<select>` dalam sebuah ekstra `<div>`. Ini penting karena Chosen akan menambahkan DOM elemen lainnya tepat setelah `<select>` node yang kita berikan pada itu. Namun, sejauh yang React ketahui, `<div>` hanya selalu memiliki satu *children*. Ini adalah bagaimana kita memastikan update React tidak akan konflik dengan DOM node ekstra yang ditambahkan Chosen. Ini juga penting bahwa jika kamu memodifikasi DOM diluar alur React, kamu harus memastikan React tidak punya alasan untuk menyentuh DOM node tersebut.
...
Kamu dapat memiliki komponen terisolasi sebanyak yang kamu suka, dan menggunakan `ReactDOM.render()` untuk merendernya pada kontainer DOM yang berbeda. Sedikit demi sedikit, saat anda mengonversi lebih banyak aplikasi ke React, kamu akan bisa mengkombinasikannya kepada komponen yang lebih besar, dan memindahkan beberapa dari `ReactDOM.render()` memanggil hirarki.
...
Untuk mendemonstrasikan bagaimana cara menggunakannya, kita akan mengkoneksikan sebuah komponen React `NameInput` pada sebuah model Backbone, dan memperbaharui atribut `firstName` setiap kali perubahan masukan:
