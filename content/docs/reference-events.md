...
`SyntheticEvent`, sebuah pembungkus lintas _browser_ yang membungkus _event_ bawaan _browser_ untuk di oper ke _event handler_. `SyntheticEvent` memiliki antarmuka yang sama dengan _event_ bawaan _browser_, termasuk `stopPropagation()` dan `preventDefault()`, namun _event_ ini memiliki fungsionalitas yang sama untuk semua _browser_.

Jika untuk alasan tertentu Anda memerlukan fungsionalitas _event_ bawaan _browser_, gunakan atribut `nativeEvent` untuk mengaksesnya. Setiap objek `SyntheticEvent` memiliki atribut-atribut ini:
...
> Catatan:
>
> Jika Anda ingin mengakses isi _event_ secara asinkron, Anda perlu memanggil fungsi `event.persist()` dalam _event_, yang akan mengeluarkan _synthetic event_ dari kumpulan sehingga acuan terhadap _event_ tersebut dapat dipertahakan melalui kode pengguna.
...
_Event_ penunjuk belum didukung oleh semua browser (saat penulisan artikel ini, _browser_ yang mendukung terdiri atas: Chrome, Firefox, Edge, dan Internet Explorer). React tidak mengisi dukungan untuk _browser_ lain karena akan meningkatkan ukuran berkas `react-dom` secara singnifikan.

Jika aplikasi Anda memerlukan _event_ pointer, kami merekomendasikan Anda menambah _event_ penunjuk dari pihak ketiga.
