...
---
 ...
 React adalah, dalam opini kami, cara paling utama dalam membangun aplikasi besar dengan JavaScript yang cepat. Ia telah men-scale dengan baik untuk kami di Facebook dan Instagram.
...
## Mulailah Dengan Sebuah Rancang Bangun {#mulailah-dengan-sebuah-rancang-bangun}
...
## Langkah 1: Bagi UI Menjadi Sebuah Hierarki Komponen {#langkah-1-bagi-ui-menjadi-sebuah-hierarki-komponen}
...
Karena Anda seringkali akan menampilkan sebuah model data JSON kepada pengguna, Anda akan menemukan bahwa jika model Anda dibangun dengan benar, maka UI (dan demikian juga struktur komponen Anda) akan dapat menyesuaikan dengan baik. Ini karena UI dan model data cenderung mengikuti *arsitektur informasi* yang sama, yang berarti memisahkan UI Anda ke dalam komponen-komponen seringkali sangat mudah. Bagi saja komponen-komponen Anda untuk merepresentasikan satu bagian dari model data Anda.
...
Anda akan melihat di sini bahwa kita memiliki lima komponen di aplikasi simpel kita. Kami telah mencetak miring data yang direpresentasikan oleh tiap komponen.
  ...
  2. **`SearchBar` (biru):** menerima semua *inputan pengguna*
  3. **`ProductTable` (hijau):** menampilkan dan memfilter *koleksi data* berdasarkan *inputan pengguna*
  4. **`ProductCategoryRow` (biru muda):** menampilkan judul untuk setiap *kategori*
  5. **`ProductRow` (red):** menampilkan sebuah baris untuk setiap *produk*
...
### Sedikit Selingan: Props vs State {#sedikit-selingan-props-vs-state}
...
## Langkah 3: Identifikasi Representasi Minimal (namun komplit) dari State UI {#langkah-3-identifikasi-representasi-minimal-namun-komplit-dari-state-ui}
...
Mari kita telaah satu per satu dan tentukan yang mana merupakan state. Jawab tiga pertaan berikut mengenai setiap bagian data:
...
## Langkah 4: Identifikasi Dimana State Anda Berada {#langkah-4-identifikasi-dimana-state-anda-berada}
...
## Langkah 5: Tambahkan Aliran Data Sebaliknya {#langkah-5-tambahkan-aliran-data-sebaliknya}
